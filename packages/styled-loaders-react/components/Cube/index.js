...
import { cube } from 'styled-loaders-common/utils/animations';
...
const Cube = ({ color, duration, size, cubeSize }) => {
	...
	const Cube = DefaultCube.extend`animation-delay: -0.9s;`;

	return (
		<Spinner size={size}>
			<DefaultCube color={color} cubeSize={cubeSize} duration={duration} />
			<Cube color={color} cubeSize={cubeSize} duration={duration} />
		</Spinner>
	);
};
